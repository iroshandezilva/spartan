...
CI mirrors these as separate steps in `.github/workflows/validate.yml`. Changing the `validate` script means changing that workflow in the same commit. Branch protection is applied to `main` as a ruleset exactly as `.github/BRANCH_PROTECTION.md` specifies, and was verified on pull request #1: a failing `validate` job blocks the merge.
